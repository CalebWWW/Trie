...
public class TrieSet implements Set<String> {
    ...
    private class TrieNode {
        ...
        public TrieNode find(String item, int index, TrieNode current) {
            if(index == 0)
            	return current;
        	for (int i = 0; i < item.length(); i++) {
            	int j = c2i(item.charAt(i));
        ...
        public TrieNode remove(String item) {
        	...
        	children[index] = children[index].remove(item);	
        ...
        public boolean isDead(TrieNode current) {
